...
function getDbPath() {
  ...
  return path.join(dataDir, 'mydatabase.db');
}
...
async function initializeDb() {
  ...
  try {
    const toSqlStringLiteral = (value) => `'${value.replace(/'/g, "''")}'`;
    ...
    const hasColumn = (columnName) => settingsColumns.some((column) => column.name === columnName);
    const selectColumnOrDefault = (columnName, fallbackSql) =>
    ...
    const templatesCount = await db.get('SELECT COUNT(*) as count FROM prompt_templates');
    ...
    const currentSettings = await db.get('SELECT selectedModel FROM settings WHERE id = 1');
    if (currentSettings) {
      if (!SUPPORTED_GEMINI_MODELS.has(currentSettings.selectedModel)) {
